...
read -p "Enter the collector domain (default collector.sample.io): " COLLECTOR_DOMAIN
# if collector domain is null, set default domain
if [ -z "$COLLECTOR_DOMAIN" ]; then
  COLLECTOR_DOMAIN="collector.sample.io"
fi
...
do
  ...
  echo "        server MPMApi0$i $BACKEND_SERVER_IP:$BACKEND_SERVER_PORT cookie s1 check ssl verify none inter 5000 fastinter 1000 rise 1 fall 1 weight 1" | tee -a $HAPROXY_CONFIG/haproxy.cfg > /dev/null
done

echo "[OFFLINE-INSTALL] HAProxy configuration is done"