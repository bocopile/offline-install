...
# mysql version 5.7.x
echo "[OFFLINE-INSTALL] Re-initializing MySQL"
...
sleep 40
echo "[OFFLINE-INSTALL] Your mysql temporary password is: $(grep -oP "temporary password is generated for root@localhost: \K.*" $MYSQL_LOG/error.log)"
echo "[OFFLINE-INSTALL] Please change your mysql password"
...
#   echo "[OFFLINE-INSTALL] MySQL password is changed"
#   # check if changed password is correct
#   mysql --socket=$MYSQL_SOCKET -u root -p$NEWPASSWORD -e "exit"
#   if [ $? -eq 0 ]; then
#     echo "[OFFLINE-INSTALL] MySQL password is correct"
#     # Create new user for mysql
#     read -p "Enter your new mysql username (default sample): " NEWUSERNAME
...
#     if [ $? -eq 0 ]; then
#       echo "[OFFLINE-INSTALL] MySQL user is created"
#     else
#       echo "[OFFLINE-INSTALL] MySQL user is not created"
...
#     fi
#   else
#     echo "[OFFLINE-INSTALL] MySQL password is incorrect"
#   fi
# fi


echo "[OFFLINE-INSTALL] MySQL configuration is done"